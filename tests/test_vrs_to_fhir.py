...
def test_full_allele_translator_returns_expected(vrs_allele_instance, vrs_to, fhir_allele_instance):
    fhir_obj = vrs_to.translate_allele_to_fhir(vrs_allele_instance)
    assert isinstance(fhir_obj, FhirAllele)
    assert fhir_obj.model_dump(exclude_none=True) == fhir_allele_instance.model_dump(exclude_none=True)
